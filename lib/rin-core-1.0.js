...
(function (rin) {
    ...
    rin.util = {
        ...
        calculateReadingDuration: function (content) {
            var sentences = this.getSentenceArray(content);
            var totalTime = 0;
            for (var i = 0; i < sentences.length; i += 2) {
                var line = sentences[i] + ((i + 1 === sentences.length) ? "" : sentences[i + 1]);
                totalTime += this.timeForSentence(line);
            }

            return totalTime;
        },

        getSentenceArray: function (content) {
            var sentences = content.split(/([\\\?.!;]+)/);
            return sentences;
        },

        timeForSentence: function (line) {
            var words = line.split(' ');
            var averageWordTime = 0.44;
            return words.length * averageWordTime;
        },
            ...
            dataProxy.getRinDataAsync(self.narrativeUrl,
                ...
                function (rinData) {
                    if (rinData && !rinData.error) {
                        ...
                        var inputJs = js_beautify(JSON.stringify(rinData));
                        var textArea = $("#txtArea");
                        if (textArea) {
                            textArea.val(inputJs);
                        }
...
(function (rin) {
    ...
    rin.internal.DemoRinDataProxy.prototype = {
        ...
        getRinDataAsync: function (narrativeUrl, onSetStatusMessage, onComplete) {
            ...
            var dataType = "json";
            var jsoncallback = this.getUrlParam(narrativeUrl, "jsonpcallback");

            if (jsoncallback) {
                dataType = "jsonp";
                narrativeUrl = narrativeUrl.substr(0, narrativeUrl.indexOf("&jsonpcallback"));
            }
