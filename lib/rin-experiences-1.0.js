...
(function () {
    /*global $:true*/
    var SideBarExperienceProvider = (function () {
        function SideBarExperienceProvider(orchestrator, esData) {
            this.stateChangedEvent = new rin.contracts.Event();
            this.orchestrator = orchestrator;
            this.esData = esData;

            this._userInterfaceControl = document.createElement("div");
            $(this._userInterfaceControl).css("width", "100%").css("height", "100%").css("position", "absolute");
        ...
        SideBarExperienceProvider.prototype = {
            load: function (experienceStreamId) {
                var self = this;
                this._loadContent(function (content) {
                    if (!content) return self.setState(rin.contracts.experienceStreamState.error);

                    self._elementHtml = content;
                    self._isHtmlLoaded = true;
                    self._userInterfaceControl.appendChild(rin.util.createElementWithHtml(self._elementHtml).firstChild);
                    self._userInterfaceControl.hide();
                    $(".sidebarCLoseButton", self._userInterfaceControl).click(function (e) {
                        self._userInterfaceControl.hide();
                        self.orchestrator.play();
                    });
                    self.loadCollectionJSON(function () {
                        $("#title", self._userInterfaceControl).html(self.collectionData.title);
                        for (var itemId in self.collectionData.items) {
                            var item = self.collectionData.items[itemId];
                            if (item.srcType === 'Media') {
                                $("#mediaDiv", self._userInterfaceControl).append('<a id=' + itemId + ' class="sidebarThumbnailImage"><img src="' + item.thumbnailMedia + '" /></a>');
                                var clickedImg = document.getElementById(itemId);
                                if (clickedImg) {
                                    clickedImg.onclick = function (e) {
                                        var id = e.target.parentElement.id;
                                        var args = {};
                                        args.orchestrator = self.orchestrator;
                                        args.host = self;
                                        args.sourceItem = {};

                                        var item = rin.internal.List.prototype.firstOrDefault.call(self.eaData, function (ea) { return ea.id === id; });
                                        if (item) {
                                            args.sourceItem.data = item.data;
                                            args.sourceItem.seekTime = item.seekTime;
                                            args.sourceItem.screenplay = item.screenplay;
                                            rin.interactionBehaviorService.executeInteractionBehavior(item.defaultInteractionBehavior, args);
                                        }
                                    };
                                }
                            }
                            else if (item.srcType === 'MainText') {
                                $("#sidebarText", self._userInterfaceControl).html(item.text);
                            }
                            else if (item.srcType === 'RelatedDataText') {
                                if (item.link) {
                                    $("#sidebarRelatedDataDiv", self._userInterfaceControl).append('<a cursor="pointer" target="_blank" href="' + item.link + '" id=' + itemId + ' class="sidebarRelatedArticle">' + item.text + '</a>');
                                }
                                else {
                                    $("#sidebarRelatedDataDiv", self._userInterfaceControl).append('<a cursor="pointer" id=' + itemId + ' class="sidebarRelatedArticle" href="#">' + item.text + '</a>');
                                    var clickedRelatedData = document.getElementById(itemId);
                                    if (clickedRelatedData) {
                                        clickedRelatedData.onclick = function (e) {
                                            var id = e.target.id;
                                            var args = {};
                                            args.orchestrator = self.orchestrator;
                                            args.host = self;
                    ...
                    self._loadEaData(function (data) {
                        if (!data) return self.setState(rin.contracts.experienceStreamState.error);
                        self.eaData = data;
                        self.setState(rin.contracts.experienceStreamState.ready);
                    });
            ...
            onESEvent: function (sender, eventId, eventData) {
                if (eventId === rin.contracts.esEventIds.stateTransitionEventId && eventData && eventData.labelClicked && eventData.labelId === this.esData.triggeringId) {
                    this._userInterfaceControl.unhide();
                }
            },
            orchestrator: null,
            esData: null,
            _loadContent: function (callback) {
                ...
                var htmlfile = this.orchestrator.getResourceResolver().resolveSystemResource('sideBar/content.htm');
                var self = this;
                $.ajax({
                    url: htmlfile,
                    dataType: "html",
                    error: function (jqxhr, textStatus, errorThrown) {
                        self.setState(rin.contracts.experienceStreamState.error);
                        self.orchestrator.eventLogger.logErrorEvent("Error: {0} downloading the html file: {1}", errorThrown, htmlfile);
                        callback(null);
                    },
            ...
            _loadEaData: function (callback) {
                var datafile = this.orchestrator.getResourceResolver().resolveResource('eaData.js');
                var self = this;
                $.ajax({
                    url: datafile,
                    dataType: "json",
                    error: function (jqxhr, textStatus, errorThrown) {
                        self.setState(rin.contracts.experienceStreamState.error);
                        self.orchestrator.eventLogger.logErrorEvent("Error: {0} downloading the html file: {1}", errorThrown, datafile);
                        callback(null);
                    },
                    success: function (data, textStatus, jqxhr) {
                        callback(data);
            ...
            loadCollectionJSON: function (callback) {
                //--from the es data, load the collection json
                var resourceResolver = this.orchestrator.getResourceResolver();
                var resourceName = resourceResolver.resolveResource(this.esData.resourceReferences[0].resourceId, this.esData.experienceId);

                if (resourceName) {
                    var self = this;
                    var cache = !(this.orchestrator.getPlayerConfiguration().playerMode === rin.contracts.playerMode.AuthorerPreview || self.orchestrator.getPlayerConfiguration().playerMode === rin.contracts.playerMode.AuthorerEditor);
                    rin.internal.JSONLoader.loadJSON(resourceName, function (data, jsonUrl) {
                        self.collectionData = rin.internal.JSONLoader.processCollectionJSON(jsonUrl, data[0].collection, resourceResolver, true);
                        self.setState(rin.contracts.experienceStreamState.ready);
                        callback();
                    }, function (error, jsonUrl) {
                        self.setState(rin.contracts.experienceStreamState.error);
                        self.orchestrator.eventLogger.logErrorEvent("Error: {0} downloading the json file: {1}", error, jsonUrl);
                    }, cache);
                }
            },
            _userInterfaceControl: null //HTMLElement
        };

        return SideBarExperienceProvider;
    })();

    rin.ext.registerFactory(rin.contracts.systemFactoryTypes.esFactory, "SideBarExperienceProvider",
        function (orchestrator, esData) {
            return new SideBarExperienceProvider(orchestrator, esData);
        });
}());
...
(function (rin) {
    /*global $:true*/
    var SubtitleExperienceProvider = function (orchestrator, esData) {

        SubtitleExperienceProvider.parentConstructor.apply(this, arguments);
        this.orchestrator = orchestrator;
        var self = this;
        var control = this._userInterfaceControl;
        this.esData = esData;
        $(this._userInterfaceControl).css("left", "50px").css("right", "50px").css("min-height", "5%").css("max-height", "33%").css("position", "absolute").css("bottom", "80px")
            .css("background-color", "rgba(0, 0, 0, 0.3)").css("padding", "10px").css("border-radius", "6px").css("color", "white").css("font-size", "24px");
    };

    rin.util.extend(rin.contracts.DiscreteKeyframeESBase, SubtitleExperienceProvider);

    SubtitleExperienceProvider.prototypeOverrides = {
        subtitleKeyframe: {
            "offset": "$OFFSET$",
            "data": {
                "subtitle": "$SUBTITLE$"
        ...
        load: function (experienceStreamId) {
            this.generateKeyframesForSubtitle(experienceStreamId);
            SubtitleExperienceProvider.parentPrototype.load.call(this, experienceStreamId);
            this.setState(rin.contracts.experienceStreamState.ready);
        },
        play: function (offset, experienceStreamId) {
            SubtitleExperienceProvider.parentPrototype.play.call(this, offset, experienceStreamId);
        },
        displayKeyframe: function (keyframeData, nextKeyframeData, interpolationOffset) {
            ...
            subtitleDiv.innerHTML = keyframeData.data.subtitle;
        },
        generateKeyframesForSubtitle: function(experienceStreamId) {
            this._esData.experienceStreams[experienceStreamId]["keyframes"] = [];
            var sentences = rin.util.getSentenceArray(this.esData.data.subtitle);
            var totalTime = 0;
            for (var i = 0; i < sentences.length; i += 2) {
                var line = sentences[i] + ((i + 1 === sentences.length) ? "" : sentences[i + 1]);
                var subtitleKF1 = JSON.stringify(this.subtitleKeyframe).replace("$OFFSET$", totalTime).replace("$SUBTITLE$", line);
                this._esData.experienceStreams[experienceStreamId].keyframes.push(rin.util.parseJSON(subtitleKF1));
                totalTime += rin.util.timeForSentence(line);
            }
        },
        getState: function () {
            return this._state;
        ...
        orchestrator: null,
        esData: null,
        _userInterfaceControl: rin.util.createElementWithHtml("<div id=\"subtitleDiv\"></div>")
    };

    rin.util.overrideProperties(SubtitleExperienceProvider.prototypeOverrides, SubtitleExperienceProvider.prototype);

    rin.ext.registerFactory(rin.contracts.systemFactoryTypes.esFactory, "SubtitleExperienceProvider",
        function (orchestrator, esData) {
            return new SubtitleExperienceProvider(orchestrator, esData);
        });
})(window.rin);
(function () {
    /*global $:false */
    ...
    var LabelExperienceProvider = (function () {
        function LabelExperienceProvider(orchestrator, esData) {
            this.stateChangedEvent = new rin.contracts.Event();
            this.orchestrator = orchestrator;
            this.esData = esData;
            this._uiControl = convertToHtmlDom(ELEMENTHTML).firstChild;
            this._uiControl.id = esData.experienceId;

            var self = this;
            setTimeout(function () {
                self.orchestrator.getESItems().foreach(function (es) {
                    if (es.experienceId === self.esData.experienceId) {
                        self.orchestrator.onESEvent(rin.contracts.esEventIds.setTimeMarkers, [es.beginOffset]);
                    }
                });
            }, 0);
        }

        LabelExperienceProvider.prototype = {
            load: function (experienceStreamId) {
            ...
            addedToStage: function () {
                var self = this;
                var btn = $('#' + this._uiControl.id);
                if (self.esData.data && self.esData.data.placement) {
                    if (self.esData.data.placement.x) {
                        btn.css('background-position-x', self.esData.data.placement.x);
                    }

                    if (self.esData.data.placement.y) {
                        btn.css('background-position-y', self.esData.data.placement.y);
                    }

                    if (self.esData.data.size && self.esData.data.size.width) {
                        self._uiControl.style.width = self.esData.data.size.width;
                    }

                    if (self.esData.data.size && self.esData.data.size.height) {
                        self._uiControl.style.height = self.esData.data.size.height;
                    }
                }

                if (btn) {
                    if (self.esData && self.esData.data && self.esData.data.icon) {
                        var iconPath = self.orchestrator.getResourceResolver().resolveResource(self.esData.data.icon);
                        btn.css('background-image', 'url("' + iconPath + '")');
                    }
                }

                document.getElementById(this._uiControl.id).onclick = function () {
                    if (self.orchestrator) {
                        self.orchestrator.pause();
                        if (self.esData && self.esData.data && self.esData.data.clickAction && self.esData.data.clickAction.data) {
                            var args = {};
                            args.orchestrator = self.orchestrator;
                            args.host = self;
                            args.sourceItem = {};
                            args.sourceItem.data = self.esData.data.clickAction.data;
                            args.sourceItem.src = self.esData.data.clickAction.data.src;
                            args.sourceItem.srcType = self.esData.data.clickAction.data.srcType;
                            rin.interactionBehaviorService.executeInteractionBehavior(self.esData.data.clickAction.defaultInteractionBehavior, args);
                        }
                        else {
                            self.orchestrator.onESEvent(rin.contracts.esEventIds.stateTransitionEventId, { labelClicked: true, labelId: self.esData.data.labelId });
                        }
                    }
                };
            },
        ...
        return LabelExperienceProvider;
    })();

    rin.ext.registerFactory(rin.contracts.systemFactoryTypes.esFactory, "LabelExperienceProvider",
        function (orchestrator, esData) {
            return new LabelExperienceProvider(orchestrator, esData);
        });
    ...
    var ELEMENTHTML = '<div class="bingSquareArtifact"></div>';
}());
/*!
* RIN Experience Provider JavaScript Library v1.0
* http://research.microsoft.com/rin
...
(function (rin) {
    ...
    rin.OverlayES = function (orchestrator, esData) {
        ...
        if (esData.data.content) {
            this._collectionData = esData.data.content[0].collection;
...
(function (rin) {
    ...
    rin.internal.JSONLoader = {
        ...
        processCollectionJSON: function (jsonUrl, collectionData, resourceResolver, resolveIncludes) {
            ...
            if (collectionData.layout && collectionData.layout.groups) {
...
(function (rin) {
    ...
    rin.internal.dRinDataGenerator = {
        _narrativeData: {
            "version": 1.0,
            "defaultScreenplayId": "SCP1",
            "screenplayProviderId": "MicrosoftResearch.Rin.DefaultScreenPlayInterpreter",
            ...
            "screenplays": {
                "SCP1": {
                    "data": {
                        ...
                        {
                            "experienceId": "TwoDLayoutEngine-1Text",
        ...
        esSrcTypeToProviderDictionary: {
            ...
            "wall": "MicrosoftResearch.Rin.WallExperienceStream",
            "map": "MicrosoftResearch.Rin.MapExperienceStream",
            "subtitle": "SubtitleExperienceProvider"
        },
        imagesESData: {
            "id": "$ID$",
            "linkType": "image",
            "srcType": "ZoomableImage",
            "src": "$SRC$",
            "begin": "$BEGIN$",
            "duration": "$DURATION$"
        },
                ...
                "state": {
                    "viewport": {
                        "region": {
                            ...
                            "span": {
                                "x": 2.1450805664063068,
                                "y": 0.74923373572010021
                            }
                        }
                    },
                    "map": {
        ...
        getUrlParam: function (sPageURL, sParam) {
        ...
        getExperienceStreams: function (queryParams) {
            var title = this.getUrlParam(queryParams, "?title") || "";
            var description = this.getUrlParam(queryParams, "description") || "";
            var esData = JSON.stringify(this._narrativeData);
            esData = esData
                .replace('$NARRATIVE_TITLE$', title)
                .replace('$NARRATIVE_DESCRIPTION$', description);

            this._narrativeData = rin.util.parseJSON(esData);

            var i = 0;
            i = this.createMapES(queryParams, i);
            i = this.createImageES(queryParams, i);
            i = this.createVideoES(queryParams, i);
            i = this.createSubtitleES(queryParams, i);
            i = this.createAudioES(queryParams, i);

            if (i !== 0) {
                delete this._narrativeData.providers.$ESPROVIDER$;
                delete this._narrativeData.resources.$RESOURCEREFID$;
                delete this._narrativeData.experiences.$ESID$;
                this._narrativeData.screenplays.SCP1.data.experienceStreamReferences.splice(0, 1);

                var maxDuration = this.getMaxDurationOfESs();
                this.setNarrativeDuration(maxDuration);

                return this._narrativeData;
            }

            return null;
        },

        createMapES: function (queryParams, i) {
            var mapData = this.getUrlParam(queryParams, "map");
            if (mapData) {
                var mapLatLong = mapData.split(",");
                if (mapLatLong.length === 2) {
                    var mapESTemplate = JSON.stringify(this.mapESData);
                    var id = "map_" + Math.floor(Math.random() * 1000).toString();
                    mapESTemplate = mapESTemplate
                        .replace("$ID$", id)
                        .replace("$SRC$", "")
                        .replace("$BEGIN$", 10 * i)
                        .replace("$DURATION$", 10);

                    var mapESJson = rin.util.parseJSON(mapESTemplate);
                    var mapKeyframeTemplate = JSON.stringify(this.mapESKeyframe);
                    mapKeyframeTemplate = mapKeyframeTemplate
                    .replace("$LAT$", mapLatLong[0])
                    .replace("$LONG$", mapLatLong[1]);

                    this.appendES(mapESJson, i, rin.util.parseJSON(mapKeyframeTemplate));
                    i++;
                }
            }
        ...
        createImageES: function (queryParams, i) {
            var imagesCSV = this.getUrlParam(queryParams, "images");
            if (imagesCSV) {
                var imageSrcArr = imagesCSV.split(",");
                for (var m = 0; m < imageSrcArr.length; m++) {
                    var imagesESTemplate = JSON.stringify(this.imagesESData);
                    var id = "image_" + Math.floor(Math.random() * 1000).toString();
                    var src = imageSrcArr[m];
                    imagesESTemplate = imagesESTemplate
                        .replace("$ID$", id)
                        .replace("$SRC$", src)
                        .replace("$BEGIN$", 10 * (m + i))
                        .replace("$DURATION$", 10);

                    var imageESJson = rin.util.parseJSON(imagesESTemplate);
                    this.appendES(imageESJson, m + i);
                }
        ...
        createVideoES: function (queryParams, i) {
            var videosCSV = this.getUrlParam(queryParams, "videos");
            if (videosCSV) {
                var j = 0;
                var videoSrcArr = videosCSV.split(",");
                var currentOffset = 10 * i;
                for (j = 0; j < videoSrcArr.length; j++) {
                    var videoESTemplate = JSON.stringify(this.videosESData);
                    var id = "video_" + Math.floor(Math.random() * 1000).toString();
                    var src = videoSrcArr[j];
                    var vDuration = (j + 1 < videoSrcArr.length) ? parseFloat(videoSrcArr[j + 1]) : NaN;
                    var duration;
                    if (isNaN(vDuration)) {
                        duration = 10;
                    }
                    ...
                    videoESTemplate = videoESTemplate
                        .replace("$ID$", id)
                        .replace("$SRC$", src)
                        .replace("$BEGIN$", currentOffset)
                        .replace("$DURATION$", duration);

                    currentOffset += duration;

                    var videoESJson = rin.util.parseJSON(videoESTemplate);
                    this.appendES(videoESJson, i + j);
                }
        ...
        createAudioES: function (queryParams, i) {
            var bgNarrativeAudio = this.getUrlParam(queryParams, "audios");
            if (bgNarrativeAudio) {
                var j = 0;
                var audioSrcArr = bgNarrativeAudio.split(",");
                var currentOffset = 0;
                for (j = 0; j < audioSrcArr.length; j++) {
                    var bgNarrativeAudioESTemplate = JSON.stringify(this.audioESData);
                    var id = "bgNarrativeAudio_" + Math.floor(Math.random() * 1000).toString();

                    var src = audioSrcArr[j];
                    var vDuration = (j + 1 < audioSrcArr.length) ? parseFloat(audioSrcArr[j + 1]) : NaN;
                    var duration;
                    if (isNaN(vDuration)) {
                        duration = "ESTIMATEDDURATION";
                    }
                    ...
                    bgNarrativeAudioESTemplate = bgNarrativeAudioESTemplate
                        .replace("$ID$", id)
                        .replace("$SRC$", src)
                        .replace("$BEGIN$", currentOffset)
                        .replace("$DURATION$", duration);
                    ...
                    var bgNarrativeAudioESJson = rin.util.parseJSON(bgNarrativeAudioESTemplate);
                    this.appendES(bgNarrativeAudioESJson, i + j);
                }
        ...
        createSubtitleES: function (queryParams, i) {
            var subtitle = this.getUrlParam(queryParams, "subtitle");
            if (subtitle) {
                var subtitleESTemplate = JSON.stringify(this.subtitleESData);
                var id = "subtitle_" + Math.floor(Math.random() * 1000).toString();
                var duration = rin.util.calculateReadingDuration(subtitle);
                subtitleESTemplate = subtitleESTemplate
                    .replace("$ID$", id)
                    .replace("$SUBTITLE$", subtitle)
                    .replace("$BEGIN$", 0)
                    .replace("$DURATION$", duration);

                var subtitleESJson = rin.util.parseJSON(subtitleESTemplate);
                i++;
                this.appendES(subtitleESJson, i);
            }

            return i;
        },

        getMaxDurationOfESs: function () {
            var maxDuration = -1;
            var scpExperiences = this._narrativeData.screenplays.SCP1.data.experienceStreamReferences;
            for (var i = 1; i < scpExperiences.length; i++) {
                if (parseFloat(scpExperiences[i].begin) + parseFloat(scpExperiences[i].duration) > maxDuration) {
                    maxDuration = parseFloat(scpExperiences[i].begin) + parseFloat(scpExperiences[i].duration);
                }
            }
        ...
        setNarrativeDuration: function (maxDuration) {
            var esData = JSON.stringify(this._narrativeData);
            var find = 'ESTIMATEDDURATION';
            var re = new RegExp(find, 'g');
            esData = esData.replace(re, maxDuration);
            this._narrativeData = rin.util.parseJSON(esData);
        },

        appendES: function (esJSON, i, keyframes) {
            var esProviderTemplate = jQuery.extend(true, {}, this._narrativeData.providers.$ESPROVIDER$);
            esProviderTemplate.name = esProviderTemplate.name + i;
            this._narrativeData.providers["$ESPROVIDER$" + i] = esProviderTemplate;

            var resourceRefTemplate = jQuery.extend(true, {}, this._narrativeData.resources.$RESOURCEREFID$);
            resourceRefTemplate.resourceId = resourceRefTemplate.resourceId + i;
            resourceRefTemplate.uriReference = resourceRefTemplate.uriReference + i;
            this._narrativeData.resources["$RESOURCEREFID$" + i] = resourceRefTemplate;

            var esTemplate = jQuery.extend(true, {}, this._narrativeData.experiences.$ESID$);
            esTemplate.providerId = "$ESPROVIDER$" + i;
            var esTemplateResourceRefTemplate = jQuery.extend(true, {}, esTemplate.resourceReferences[0]);
            esTemplateResourceRefTemplate.resourceId = esTemplateResourceRefTemplate.resourceId + i;
            esTemplate.resourceReferences.push(esTemplateResourceRefTemplate);
            esTemplate.resourceReferences.splice(0, 1);
            if (keyframes) {
                esTemplate.experienceStreams.defaultStream["keyframes"] = keyframes;
            }

            if (esJSON.subtitle) {
                esTemplate.data["subtitle"] = "$SUBTITLE$" + i;
            }

            this._narrativeData.experiences["$ESID$" + i] = esTemplate;

            var scrPlayExperienceStreamReferencesTemplate = jQuery.extend(true, {}, this._narrativeData.screenplays.SCP1.data.experienceStreamReferences[0]);
            scrPlayExperienceStreamReferencesTemplate.experienceId = scrPlayExperienceStreamReferencesTemplate.experienceId + i;
            scrPlayExperienceStreamReferencesTemplate.begin = scrPlayExperienceStreamReferencesTemplate.begin + i;
            scrPlayExperienceStreamReferencesTemplate.duration = scrPlayExperienceStreamReferencesTemplate.duration + i;
            this._narrativeData.screenplays.SCP1.data.experienceStreamReferences.push(scrPlayExperienceStreamReferencesTemplate);

            var esData = JSON.stringify(this._narrativeData);
            this.processExperienceStream(esJSON, "", esData, i);
        },

        processExperienceStream: function (context, themeName, esData, index) {
            if (context === undefined)
                return;
            ...
            esData = this.replaceAll('$ESID$' + index, esId, esData);
            esData = this.replaceAll("$ESPROVIDER$" + index, providerName, esData);
            esData = esData.replace('$RESOURCEREF$' + index, context.src)
                           .replace('$CONTENTTYPE$', context.srcType)
                           .replace('$ASPECTRATIO$', aspectratio)
                           .replace('$BEGIN$' + index, begin)
                           .replace('$DURATION$' + index, duration)
                           .replace('$STARTOFFSET$', startOffset)
                           .replace('$ENDOFFSET$', endOffset)
                           .replace('$VOLUME$', context.volume || 1)
                           .replace('$SUBTITLE$' + index, context.subtitle);
